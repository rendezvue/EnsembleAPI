...
CInterfaceControl::CInterfaceControl(void)
{
    ...
    CEthernetClientControlData::getInstance() ;
}
...
int CInterfaceControl::Task_Load(void)
{
	...
	return Task_Load(vec_str_db_id) ;
}
...
int CInterfaceControl::Tool_Option_InspectColor_Histogram_GetImage(const std::string option_id, const int color_num, const int type_option, int& width, int& height, ImageBuf* out_buf)
{
	boost::unique_lock<boost::mutex> scoped_lock(mutex);
...
int CInterfaceControl::Tool_Option_ColorCompare_GetInspectLevel(const std::string option_id) 
{
	boost::unique_lock<boost::mutex> scoped_lock(mutex);
	...
	unsigned int command = ENSEMBLE_TOOL_OPTION_INSPECT_COLORCOMPARE_GET_INSPECT_LEVEL;
...
int CInterfaceControl::Tool_Option_ColorCompare_SetInspectLevel(const std::string option_id, const int level) 
{
	boost::unique_lock<boost::mutex> scoped_lock(mutex);
	...
	unsigned int command = ENSEMBLE_TOOL_OPTION_INSPECT_COLORCOMPARE_SET_INSPECT_LEVEL;
...
int CInterfaceControl::Tool_Option_ColorCompare_GetUseAmp(const std::string option_id) 
{
	boost::unique_lock<boost::mutex> scoped_lock(mutex);
	...
	unsigned int command = ENSEMBLE_TOOL_OPTION_INSPECT_COLORCOMPARE_GET_USE_AMP;
...
int CInterfaceControl::Tool_Option_ColorCompare_SetUseAmp(const std::string option_id, const bool b_use)
{
	boost::unique_lock<boost::mutex> scoped_lock(mutex);
	...
	unsigned int command = ENSEMBLE_TOOL_OPTION_INSPECT_COLORCOMPARE_SET_USE_AMP;
...
int CInterfaceControl::Calibration_GetImage(const std::string job_id, int index, const int type_option, int& width, int& height, ImageBuf* out_buf)
{
    boost::unique_lock<boost::mutex> scoped_lock(mutex);
	...
	vec_send_data.push_back(width) ;
	vec_send_data.push_back(height) ;
    p_cls_ethernet_control_data->Send(p_socket, command, job_id, &vec_send_data) ;
    int image_buf_size= p_cls_ethernet_control_data->ReceiveImage(p_socket, command, width, height, out_buf) ;
...
int CInterfaceControl::GetImage(const int option, std::string id, const int type_option, int& width, int& height, ImageBuf* out_buf)
{
    boost::unique_lock<boost::mutex> scoped_lock(mutex);
...
int CInterfaceControl::GetResultImage(const std::string id, const int type_option, int& width, int& height, ImageBuf* out_buf)
{
	boost::unique_lock<boost::mutex> scoped_lock(mutex);
...
std::string CInterfaceControl::DB_Get_List(void)
{
	boost::unique_lock<boost::mutex> scoped_lock(mutex);
...
int CInterfaceControl::Camera_Capture_SW_Trigger(void)
{
    boost::unique_lock<boost::mutex> scoped_lock(mutex);
...
int CInterfaceControl::Camera_Capture_HW_Trigger(void)
{
	boost::unique_lock<boost::mutex> scoped_lock(mutex);
    ...
    unsigned int command = ENSEMBLE_CAMERA_CAPTURE_HW_TRIGGER  ;
...
int CInterfaceControl::Digital_IO_GetIn(void)
{
    boost::unique_lock<boost::mutex> scoped_lock(mutex);
...
int CInterfaceControl::Digital_IO_SetOut(int pin_num, int pin_val)
{
    boost::unique_lock<boost::mutex> scoped_lock(mutex);
